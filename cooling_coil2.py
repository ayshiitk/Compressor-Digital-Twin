...
class CopperCoolingCoil_TwinFan:
    ...
    def analyze_coil(self, m_dot_kg_s, P_in_pa, T_in_k, T_amb_k, selected_length=3.36):
        ...
        T_chamber_k = T_amb_k + 4.5 # Assume chamber air is at ambient temperature
        # Target threshold: 1 Degree above ambient
        target_approach = 1.0
        T_target_out_k = T_chamber_k + target_approach
        ...
        h_in, h_out, Re_in, Dean, rho_in, v_in = self._calculate_heat_transfer_coefficients(
            m_dot_kg_s, P_in_pa, T_mean_guess, T_chamber_k
        )
        ...
        if T_in_k <= T_target_out_k:
            return T_in_k, T_chamber_k - 273.15, P_in_pa  # No cooling needed; return inputs as outputs
          
        C_min = m_dot_kg_s * self.cp_air
        if C_min < 1e-6:   # ← ADD THIS GUARD
            return T_in_k, T_chamber_k, P_in_pa

        NTU_req = -math.log(target_approach / (T_in_k - T_chamber_k))
        ...
        T_final_k = T_chamber_k + (T_in_k - T_chamber_k) * math.exp(-NTU_eval)
        ...
        T_ambient_out_celsius = (T_chamber_k + delta_T_ambient) - 273.15
        ...
        return T_final_k,  P_out_pa, delta_p_pa
...
if __name__ == "__main__":
    ...
    for flow_slpm, pin_gauge_bar, tin_celsius in test_points:
        ...
        Tout, Tamb_out, Pout_pa, dP_pa, flow_out, _, _ = coil_solver.analyze_coil(
            mass_flow,
            pin_abs_pa,
            temperature_inlet,
            ambient_room
